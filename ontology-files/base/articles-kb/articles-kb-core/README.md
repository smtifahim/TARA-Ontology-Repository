...
- Documentation generators now live at repo root under **`ontology-generator/docs-generator/`** (shared across ontologies, not just this one). The shared HTML engine is `lib/ttl_to_html.py`; each ontology gets its own `<ontology>-html/` folder next to it holding a thin per-ontology config script. For this ontology that folder is **`articles-kb-metadata-html/`**, containing:
  - `generate_articles_kb_core_html.py` — a thin `OntologyDocConfig` on top of `lib/ttl_to_html.py`, which writes the interactive HTML ontology browser (search/pulldown-autocomplete, expandable trees with a resizable sidebar, browser-history navigation, deep-linkable `#curie` fragments) as `index.html` + `styles.css` to `docs/articles-kb-core/` at the repo root by default, served by GitHub Pages. Its `articles-kb-core-default-settings.json` sibling holds the baked-in default panel/field arrangement for all viewers.
  - `generate_documentation.py` — writes `tara-articles-kb-core-doc.md` alongside it: class hierarchy, full `tara-kb:hasTARAArticlesMetadata` property tree with domain/range, and a computed Data Quality Notes section.
  - Run either with `python3 ontology-generator/docs-generator/articles-kb-metadata-html/<script>.py [--ttl PATH] [--out PATH]`.
