...
Collects MONDO and HP URIs from one or more CSV files (see CONDITIONS_CSVS),
all read from the same column (MONDO-OR-HP-Term-URI, values often
comma-separated), and merges them into a single unique seed-URI list per
ontology.
...
# ---------------------------------------------------------------------------
# Source CSVs (seed URIs)
# ---------------------------------------------------------------------------
_CONDITIONS_SHEETS_DIR = ( REPO_ROOT / "ontology-files" / "base" / "articles-kb" / "kb_terms_mapping"
                                     / "conditions_mapping" / "conditions_mapped_sheets")

# REPO_ROOT / "ontology-files" / "base" / "articles-kb" / "kb_terms_mapping" / "conditions_mapping" / "conditions_mapped_sheets"

CONDITIONS_CSVS = [ 
                   _CONDITIONS_SHEETS_DIR / "Disease-Conditions-112625.csv",
                   # We can add more condition CSV files here as needed
                  ]
...
def extract_seed_uris_from_csvs(csv_paths: List[Path], column: str) -> Tuple[Set[str], Set[str]]:
    """Combines MONDO/HP URIs extracted from `column` across all csv_paths into one unique set each."""
    mondo_uris: Set[str] = set()
    hp_uris: Set[str] = set()
    for csv_path in csv_paths:
        print(f"    Reading {_display_path(csv_path)} ...")
        file_mondo_uris, file_hp_uris = extract_seed_uris_from_csv(csv_path, column)
        print(f"      {len(file_mondo_uris)} MONDO URIs, {len(file_hp_uris)} HP URIs")
        mondo_uris.update(file_mondo_uris)
        hp_uris.update(file_hp_uris)
    return mondo_uris, hp_uris
...
def extract_mondo_hp(mondo_ttl_path: Path, hp_ttl_path: Path,
    ...
    dst.add((onto_uri, RDFS.comment, Literal(
        ...
        "Seed terms sourced from the MONDO-OR-HP-Term-URI column across one or more "
        "conditions-mapped CSV files. "
...
# ---------------------------------------------------------------------------
# Post-processing (standalone, can be run independently)
# ---------------------------------------------------------------------------

def postprocess_mondo_hp_ontology(input_file=None, output_file=None):
    """
    Post-processing function that can be run independently of the main import.

    Operations:
    1. For HP:0000001 (All):
       - Replace rdfs:label "All" with "HPO Root Term"
       - Add editor note (IAO:0000116) explaining the change
    2. Move HP:0000001, UBERON:0001062, and MONDO:0000001 to be directly under owl:Thing
    3. Remove all other BFO classes

    Args:
        input_file: Path to TTL file to process. If None, uses the default output file.
        output_file: Path to save post-processed file. If None, saves to imported-conditions/imported-tara-kb-mondo-hp-terms.ttl
    """
    if input_file is None:
        input_file = str(OUTPUT_TTL)

    if output_file is None:
        output_file = (REPO_ROOT / "ontology-files" / "base" / "articles-kb" / "imported-conditions" / "imported-tara-kb-mondo-hp-terms.ttl")

    # Display relative paths from repo root
    try:
        input_display = Path(input_file).relative_to(REPO_ROOT)
        output_display = Path(output_file).relative_to(REPO_ROOT)
    except (ValueError, TypeError):
        input_display = Path(input_file)
        output_display = Path(output_file)

    print(f"Loading ontology from: {input_display}")
    ...
    # Define URIs
    HP_ROOT = URIRef("http://purl.obolibrary.org/obo/HP_0000001")
    UBERON_ANATOMICAL = URIRef("http://purl.obolibrary.org/obo/UBERON_0001062")
    MONDO_ROOT_DISEASE = URIRef("http://purl.obolibrary.org/obo/MONDO_0000001")
    IAO_EDITOR_NOTE = URIRef("http://purl.obolibrary.org/obo/IAO_0000116")
    BFO_PREFIX = "http://purl.obolibrary.org/obo/BFO_"
    ROOT_CLASSES = {HP_ROOT, UBERON_ANATOMICAL, MONDO_ROOT_DISEASE}

    # 1. Process HP:0000001 (All)
    print("  Processing HP:0000001 (All)...")
    # Remove old "All" label
    for label in list(g.objects(HP_ROOT, RDFS.label)):
        g.remove((HP_ROOT, RDFS.label, label))
        print(f"    Removed old label: {label}")

    # Add new label
    new_label = Literal("HPO Root Term")
    g.add((HP_ROOT, RDFS.label, new_label))
    print(f"    Added new label: {new_label}")

    # Add editor note
    editor_note = Literal(
        "Please Note: The original rdfs:label for this term (HP:0000001) is called 'All' in HPO. "
        "The original rdfs:label is replaced with the current one to avoid ambiguity for the TARA acupoints ontology."
    )
    g.add((HP_ROOT, IAO_EDITOR_NOTE, editor_note))
    print(f"    Added editor note")

    # 2. Move root classes to be directly under owl:Thing
    print("  Moving root classes to be directly under owl:Thing...")
    for root_class in ROOT_CLASSES:
        # Remove any existing subClassOf relationships
        for parent in list(g.objects(root_class, RDFS.subClassOf)):
            g.remove((root_class, RDFS.subClassOf, parent))
            print(f"    {root_class}: removed subClassOf {parent}")

        # Add subClassOf owl:Thing
        g.add((root_class, RDFS.subClassOf, OWL_URI.Thing))
        print(f"    {root_class}: added subClassOf owl:Thing")

    # 3. Remove all other BFO classes (except the three root classes we want to keep)
    print("  Removing other BFO classes...")
    bfo_classes = []
    for s in g.subjects(RDF.type, OWL_NS.Class):
        if isinstance(s, URIRef) and str(s).startswith(BFO_PREFIX) and s not in ROOT_CLASSES:
            bfo_classes.append(s)

    removed_count = 0
    for bfo_class in bfo_classes:
        # Remove triples where this class is subject
        for p, o in list(g.predicate_objects(bfo_class)):
            g.remove((bfo_class, p, o))

        # Remove triples where this class is object (e.g., subClassOf)
        for s, p in list(g.subject_predicates(bfo_class)):
            g.remove((s, p, bfo_class))

        removed_count += 1
        if removed_count <= 5:
            print(f"    Removed: {bfo_class}")

    if removed_count > 5:
        print(f"    ... and {removed_count - 5} more BFO classes")
    print(f"    Total BFO classes removed: {removed_count}")

    # Rebind namespaces for clean output
    g.bind("MONDO", Namespace(MONDO_PREFIX))
    g.bind("HP", Namespace(HP_PREFIX))
    g.bind("UBERON", Namespace(UBERON_PREFIX))
    g.bind("IAO", Namespace(IAO_PREFIX))
    g.bind("oboInOwl", Namespace(OBOINOWL_PREFIX), override=True)
    g.bind("mondo", Namespace(MONDO_HASH_PREFIX))
    g.bind("owl", OWL_URI)
    g.bind("rdfs", RDFS)
    g.bind("rdf", RDF)
    ...
    print(f"\nDone! Post-processed ontology saved to: {output_display}")
...
def main():
    ...
    if not extract_only:
        print(f"\n[1] Collecting MONDO/HP URIs from {len(CONDITIONS_CSVS)} CSV file(s) ...")
        mondo_uris, hp_uris = extract_seed_uris_from_csvs(CONDITIONS_CSVS, URI_COLUMN)
        print(f"    {len(mondo_uris)} unique MONDO URIs, {len(hp_uris)} unique HP URIs (combined across all files)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Import MONDO/HP terms or post-process ontology")
    parser.add_argument("--postprocess", action="store_true", help="Run post-processing only (no API calls)")
    parser.add_argument("--input", type=str, help="TTL file to post-process (default: imported-tara-mondo-hp-terms.ttl)")
    parser.add_argument("--output", type=str, help="Output TTL file (default: imported-tara-kb-mondo-hp-terms.ttl)")

    args = parser.parse_args()

    if args.postprocess:
        postprocess_mondo_hp_ontology(args.input, args.output)
    else:
        main()
