'''
convert_class_iris.py — TARA Class IRI Converter (lib helper)

Converts class IRI suffixes in an RDFLib graph from the human-readable textual form used
during CSV authoring (e.g., TARA:Meridian_Acupoint) to stable, numeric TARA IDs
(e.g., TARA:TARA_0123456). The numeric suffix is derived from a SHA-256 hash of the
original text, making the conversion deterministic and collision-resistant within the
7-digit ID space.

Called automatically by generate_ontology.py after the initial graph is built from CSV.
